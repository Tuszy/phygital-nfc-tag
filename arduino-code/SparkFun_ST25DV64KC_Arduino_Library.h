...
class SFE_ST25DV64KC
{
  public:
    ...
    // Writes multiple values to mailbox
    inline bool writeToMailbox(uint8_t *const buffer, uint16_t packetLength)
    {
      return st25_io.writeMultipleBytesToBuffer(buffer, packetLength);
    }

    // Reads multiple values from mailbox
    inline bool readFromMailbox(uint8_t *const buffer, uint16_t packetLength)
    {
      return st25_io.readMultipleBytesFromBuffer(buffer, packetLength);
    }
    ...
    // Prevents RF writing to configuration
    bool lockConfiguration();

    // Enables the mailbox (need to be called only once)
    bool enableMailbox();

    // Sets the mailbox's active state
    bool setMailboxActive(bool value);

    // Gets the mailbox message length
    uint16_t getMailboxMessageLength();
};
