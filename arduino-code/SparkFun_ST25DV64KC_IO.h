...
public:
  ...
  bool readMultipleBytes(const SF_ST25DV64KC_ADDRESS address, const uint16_t registerAddress, uint8_t *const buffer, uint16_t packetLength);

  // Reads multiple bytes from mailbox into buffer uint8_t array.
  bool readMultipleBytesFromBuffer(uint8_t *const buffer, uint16_t packetLength);

  // Writes multiple bytes to register from buffer uint8_t array.
  bool writeMultipleBytes(const SF_ST25DV64KC_ADDRESS address, const uint16_t registerAddress, uint8_t *const buffer, uint16_t packetLength);

  // Writes multiple bytes to mailbox from buffer uint8_t array.
  bool writeMultipleBytesToBuffer(uint8_t *const buffer, uint16_t packetLength);
