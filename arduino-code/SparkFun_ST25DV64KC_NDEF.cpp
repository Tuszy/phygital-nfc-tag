      ...
      case readType:
        {
          ...
          theType[typeLength] = 0;                                                                                                                                        // Null-terminate the Type
          eepromAddress += typeLength;                                                                                                                                    // Point to the ID
          if ((tnf == SFE_ST25DV_NDEF_TNF_MEDIA) && (typeLength == strlen(SFE_ST25DV_WIFI_MIME_TYPE)) && (strcmp((const char *)theType, SFE_ST25DV_WIFI_MIME_TYPE) == 0)) // Check for a Type match
            loopState = matchFoundCheckRecordNo;
          else
            loopState = readAndIgnoreID;
        }
...
bool SFE_ST25DV64KC_NDEF::setLockCCFile(bool value)
{
#if CC_FILE_SIZE == 4
  if (value) {
    return st25_io.setRegisterBit(SF_ST25DV64KC_ADDRESS::SYSTEM, REG_LOCK_CCFILE, BIT_LOCK_CCFILE_LCKBCK0);
  }
  else
  {
    return st25_io.clearRegisterBit(SF_ST25DV64KC_ADDRESS::SYSTEM, REG_LOCK_CCFILE, BIT_LOCK_CCFILE_LCKBCK0);
  }
#else
  if (value) {
    return st25_io.setRegisterBit(SF_ST25DV64KC_ADDRESS::SYSTEM, REG_LOCK_CCFILE, BIT_LOCK_CCFILE_LCKBCK0 | BIT_LOCK_CCFILE_LCKBCK1);
  }
  else {
    return st25_io.clearRegisterBit(SF_ST25DV64KC_ADDRESS::SYSTEM, REG_LOCK_CCFILE, BIT_LOCK_CCFILE_LCKBCK0 | BIT_LOCK_CCFILE_LCKBCK1);
  }
#endif
}
