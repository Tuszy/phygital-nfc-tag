...
#define Curve_B_1                                           \
    {                                                       \
        0x45, 0xFA, 0x65, 0xC5, 0xAD, 0xD4, 0xD4, 0x81,     \
...
#define Curve_N_1                                           \
    {                                                       \
        0x57, 0x22, 0x75, 0xCA, 0xD3, 0xAE, 0x27, 0xF9,     \
...
#define Curve_B_5                                        \
    {                                                    \
        0x270B39432355FFB4ull, 0x5044B0B7D7BFD8BAull,    \
...
#define Curve_N_5                                        \
    {                                                    \
        0x13DD29455C5C2A3Dull, 0xFFFF16A2E0B8F03Eull,    \
            0xFFFFFFFFFFFFFFFFull, 0x00000000FFFFFFFFull \
    }
...
static int uECC_sign_with_k(const uint8_t private_key[uECC_BYTES],
                            ...
                            uint8_t signature[uECC_BYTES * 2 + 1],
                            uint32_t chainId)
{
              ...
              uint32_t chainId)
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        if (g_rng_function((uint8_t *)k, sizeof(k)))
        {
            ...
            k[uECC_WORDS] &= 0x01;
#endif
            if (uECC_sign_with_k(private_key, message_hash, k, signature, chainId))
            {
                return 1;
            }
        }
                            ...
                            uint32_t chainId)
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        ...
        if (uECC_sign_with_k(private_key, message_hash, T, signature, chainId))
        {
            return 1;
        }
